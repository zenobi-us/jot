...
updated_at: 2026-02-14T21:57:00+10:30
status: completed
epic_id: 7c9d2e1f
phase_id: 52a9f0b3
priority: medium
...
## Acceptance Criteria
- [x] Semantic search accepts the same query DSL filters as `opennotes notes search query` (data.*, path, title, links-to, linked-by).
- [x] Users can combine exclusions with `--not` semantics (e.g., `--and data.tag=epic --not data.status=archived`).
- [x] Exclusions apply to both keyword and semantic results in hybrid mode.
