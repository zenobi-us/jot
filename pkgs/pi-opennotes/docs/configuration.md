...
Complete reference for configuring `@zenobi-us/pi-opennotes`.
...
- **Type**: `string`
- **Default**: `"opennotes_"`
- **Env**: `OPENNOTES_TOOL_PREFIX`

Prefix for all tool names.

**Examples**:
```json
"toolPrefix": "opennotes_"  // opennotes_search, opennotes_list
...
- **Type**: `string`
- **Default**: `"opennotes"`
- **Env**: `OPENNOTES_CLI_PATH`

Path to OpenNotes CLI binary.

**Examples**:
```json
"cliPath": "opennotes"                      // In PATH
"cliPath": "/usr/local/bin/opennotes"       // Absolute path
"cliPath": "/home/user/go/bin/opennotes"    // User install
"cliPath": "./bin/opennotes"                // Relative path
```

**When to set**:
- CLI not in PATH
- Multiple OpenNotes versions installed
...
# Persist
echo 'export OPENNOTES_PAGE_SIZE="100"' >> ~/.bashrc
source ~/.bashrc
```
...
# Environment: OPENNOTES_PAGE_SIZE=100
...
export OPENNOTES_CLI_PATH="./dist/opennotes-dev"
export OPENNOTES_CLI_TIMEOUT="120000"  # Longer timeout for debugging
```

**Testing**:
```bash
# Smaller pages for testing pagination
export OPENNOTES_PAGE_SIZE="5"
```
...
Each notebook has `.opennotes.json` at its root.
...
{
  "plugins": [
    "@opennotes/plugin-git",
    "@opennotes/plugin-sync",
    "custom-plugin"
  ]
}
```
...
Track `.opennotes.json`:

```bash
cd ~/notes
git init
git add .opennotes.json
