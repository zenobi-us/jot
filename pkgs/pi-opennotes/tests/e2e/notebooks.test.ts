...
describeE2E("Multi-Notebook", () => {
  ...
  beforeAll(async () => {
    ...
    notebook1Path = join(tmpBase, `pi-opennotes-nb1-${Date.now()}`);
    notebook2Path = join(tmpBase, `pi-opennotes-nb2-${Date.now()}`);
    ...
    writeFileSync(
      join(notebook2Path, ".opennotes.json"),
...
`
  ...
  it("should handle notebook without views", skipIfNoCli(async () => {
    ...
    const minimalPath = join(tmpdir(), `pi-opennotes-minimal-${Date.now()}`);
    mkdirSync(minimalPath, { recursive: true });
    
    writeFileSync(
      join(minimalPath, ".opennotes.json"),
