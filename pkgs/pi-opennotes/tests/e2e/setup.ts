 ...
 * Tests will be skipped if OpenNotes CLI is not installed.
 */
...
async function checkCli(): Promise<{ available: boolean; path: string; version: string }> {
  try {
    ...
    if (exitCode === 0) {
      // Parse version from output like "OpenNotes 0.0.2"
...
export async function getE2EEnvironment(): Promise<E2EEnvironment> {
  if (!e2eEnv) {
    ...
    const testDir = join(tmpdir(), `pi-opennotes-e2e-${Date.now()}`);
...
export async function createTestNotebook(path: string): Promise<void> {
  ...
  writeFileSync(join(path, ".opennotes.json"), JSON.stringify(config, null, 2));
...
export async function execCli(
  ...
  const proc = Bun.spawn(["opennotes", ...fullArgs], {
    stdout: "pipe",
    stderr: "pipe",
  });
...
export function skipIfNoCli(testFn: () => void | Promise<void>): () => void | Promise<void> {
  return async () => {
    ...
    if (!env.cliAvailable) {
      console.log("⏭️  Skipping: OpenNotes CLI not installed");
      return;
    }
    await testFn();
  };
