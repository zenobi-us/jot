/**
 * CLI Adapter for opennotes commands
...
export class CliAdapter implements ICliAdapter {
  ...
  async checkInstallation(): Promise<InstallationInfo> {
    ...
    try {
      ...
      if (result.code === 0) {
        // Parse version from output like "opennotes version 0.10.0"
        const versionMatch = result.stdout.match(/(?:opennotes\s+)?version\s+(\S+)/i);
