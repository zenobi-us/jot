...
export class NotebookService implements INotebookService {
  ...
  async listNotebooks(): Promise<NotebooksResult> {
    ...
    const result = await this.cli.exec("jot", ["notebook", "list", "--format", "json"]);
  ...
  async getCurrentNotebook(explicitPath?: string): Promise<NotebookInfo | null> {
    ...
    const result = await this.cli.exec("jot", args);
  ...
  async validateNotebook(path: string): Promise<ValidateNotebookResult> {
    ...
    const result = await this.cli.exec("jot", [
      "notebook",
      "info",
      "--format",
      "json",
    ...
    if (result.stderr.includes(".jot.json") || result.stderr.includes("config")) {
      return {
        valid: false,
        error: "Directory is not a valid notebook (missing .jot.json)",
      };
    }
