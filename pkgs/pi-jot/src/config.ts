/**
 * Extension configuration for pi-jot
...
export interface ExtensionConfig {
  /**
   * Prefix for tool names (e.g., 'jot_search')
   * @default "jot_"
   */
  toolPrefix: string;
...
export function getConfig(pi: ExtensionAPI): ExtensionConfig {
  ...
  const envPrefix = process.env.JOT_TOOL_PREFIX;
  if (envPrefix) {
    config.toolPrefix = envPrefix;
  }

  const envPageSize = process.env.JOT_PAGE_SIZE;
  if (envPageSize) {
  ...
  const envCliPath = process.env.JOT_CLI_PATH;
  if (envCliPath) {
    config.cliPath = envCliPath;
  }

  const envTimeout = process.env.JOT_CLI_TIMEOUT;
  if (envTimeout) {
