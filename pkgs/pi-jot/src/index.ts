 ...
 * pi-jot Extension
 *
 * Integrates Jot into the pi coding agent, enabling AI assistants to:
 ...
 *
 * @see https://github.com/zenobi-us/jot
 */
...
export default function piJot(pi: ExtensionAPI): void {
  // Load configuration
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      if (!installation.installed) {
        ctx.ui.notify(
          "⚠️ Jot CLI not found. Install with: go install github.com/zenobi-us/jot@latest",
          "warning"
        );
      } else if (installation.version) {
        // Optional: show version in status
        // ctx.ui.setStatus("jot", `Jot ${installation.version}`);
      }
    } catch {
  ...
  pi.registerCommand("jot", {
    description: "Jot status and info",
...
export { JotError, ErrorCodes, type ErrorCode } from "./utils/errors";
export * from "./schemas";
