...
describe("CliAdapter", () => {
  ...
  describe("exec", () => {
    it("executes command with args", async () => {
      await adapter.exec("jot", ["notes", "search"]);
    ...
    it("adds notebook flag when provided", async () => {
      await adapter.exec("jot", ["notes", "list"], {
    ...
    it("returns CLI result", async () => {
      ...
      const result = await adapter.exec("jot", ["version"]);
    ...
    it("handles non-zero exit code", async () => {
      ...
      const result = await adapter.exec("jot", ["invalid"]);
  ...
  describe("parseJsonOutput", () => {
    ...
    it("throws JotError for invalid JSON", () => {
      expect(() => adapter.parseJsonOutput("not json")).toThrow(JotError);
    });
    ...
    it("includes stdout preview in error details", () => {
      try {
        ...
        expect(error).toBeInstanceOf(JotError);
        expect((error as JotError).code).toBe(ErrorCodes.PARSE_ERROR);
        expect((error as JotError).details?.stdout).toBeDefined();
      }
    });
  });
