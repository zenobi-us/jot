...
Comprehensive guide to using all `@zenobi-us/pi-jot` tools with detailed examples.

## Table of Contents

- [jot_search](#jot_search)
- [jot_list](#jot_list)
- [jot_get](#jot_get)
- [jot_create](#jot_create)
- [jot_notebooks](#jot_notebooks)
- [jot_views](#jot_views)
...
**Requires**: Template defined in notebook's `.jot.json`.
...
Defined in notebook's `.jot.json`:
...
| Code | Meaning | Hint |
|------|---------|------|
| `JOT_CLI_NOT_FOUND` | CLI not installed | Install Jot CLI |
...
```typescript
const result = await jot_search({ query: "test" });

if (result.error && result.code === "JOT_CLI_NOT_FOUND") {
...
```typescript
// Quick lookup
await jot_get({ path: "note.md" });  // Default timeout
...
await jot_search({ sql: `SELECT * FROM notes WHERE title = '${userInput}'` });
