...
Pi extension for [Jot](https://github.com/zenobi-us/jot) - search and manage markdown notes with AI.

## Overview

This extension integrates Jot into the [pi coding agent](https://github.com/mariozechner/pi-coding-agent), enabling AI assistants to:
...
- [Jot CLI](https://github.com/zenobi-us/jot) installed and in PATH
...
```json
{
  "config": {
    "@zenobi-us/pi-jot": {
      "toolPrefix": "jot_",    // Tool name prefix
      "defaultPageSize": 50,          // Results per page
      "cliPath": "jot",         // Path to CLI binary
...
- `JOT_TOOL_PREFIX` - Tool name prefix
- `JOT_PAGE_SIZE` - Default page size
- `JOT_CLI_PATH` - Path to CLI binary
- `JOT_CLI_TIMEOUT` - Command timeout
...
{
  error: true,
  message: "Jot CLI not found",
  code: "JOT_CLI_NOT_FOUND",
  hint: "Install with: go install github.com/zenobi-us/jot@latest",
  recoverable: true
}
```
...
The extension also registers a `/jot` command for status:
